...
Headless sync shares most settings with the official Sync plugin (it uses the official headless sync cli in the background). In the Settings tab, under "Ignis Core Plugins" heading; you can configure which remote vault to sync with, what types of files or settings you want to sync, manage excluded folders, and see the log from the sync process. For details on how these settings work, see the official Obsidian help for [Sync settings and selective syncing](https://obsidian.md/help/sync/settings) and for the [Headless Sync CLI](https://obsidian.md/help/sync/headless).
