...
describe("Obsidian 1.12.x compatibility", () => {
  it("terms/policy keep the payload shapes 1.12.7 was verified against", () => {
    window.__obsidianVersion = "1.12.7";
    try {
      expect(ipcRenderer.sendSync("terms")).toEqual({ accepted: true });
      expect(ipcRenderer.sendSync("policy")).toBeNull();
    } finally {
      window.__obsidianVersion = "1.13.7";
    }
  });
});
